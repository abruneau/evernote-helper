((global, factory) ->
  if typeof exports == 'object' and typeof module != 'undefined' then factory(exports) else if typeof define == 'function' and define.amd then define([ 'exports' ], factory) else factory(global.EvernoteHelper = global.EvernoteHelper or {})
  return
) this, (exports) ->
  exports.test = 5
  ...
  literalReplace = (inputStr, oldSubStr, newSubStr, options) ->
  ...
  exports.prepareHtml = (html) ->
    cheerio ?= require 'cheerio'
    o = cheerio.load(html)
    for div in o('.table-of-contents')
  ...
  exports.toMarkdown = (html, options) ->
    ...
    parseImage = (oimg, options) =>
      ...
      todo = o(oimg).attr("class")?.indexOf('en-todo') > -1
      if todo
        checked = o(oimg).attr("class")?.indexOf("en-todo-checked") > -1
    ...
    parseObject = (obj, options) =>
      ...
      checked = o(obj).attr("class")?.indexOf("en-todo-checked") > -1
      objText = if checked then "[x]" else "[ ]"
      objText += " " + o(obj).html()
    ...
    parseInput = (inp, options) =>
      ...
      checked = o(inp).attr("class")?.indexOf("en-todo-checked") > -1
    ...
    parsePre = (opre, options) =>
      ...
      if preTitle and preTitle.indexOf("fenced-code-block") > -1
    ...
    for div in o('div')
      if o(div).css("display") is "none" then o(div).remove()

    # remove the TOC sections
    for ul in o('ul')
      if o(ul).attr("title") is "\"table-of-contents\""
